      ...
      for( int k=1;k<=n;k++ )
       ...
       for(int l=1;l<n;l++)
         for(int m=l+1;m<=n;m++ ){
             ...
             for(int i=1;i<n;i++)
               for(int j=i+1;j<=n;j++){
                  double d3=min(pre[i][l]+dis+pre[m][j] ,pre[i][m]+dis+pre[l][j] );
                  cur=min(pre[i][j],d3 );
                  cuv1+=pre[i][j]-cur;
                  //printf("%d %d %lf %lf \n",i,j,pre[i][j],cur);
                ...
                if(cuv1>cuv)
                    ...
                    double d1=dif(nod[l].x,nod[l].y,nod[m].x,nod[m].y);
                    double d2=dif(nod[u].x,nod[u].y,nod[v].x,nod[v].y);
                     if(d1<d2){
                           u=l;v=m;
                           cuv=cuv1;
                         }
